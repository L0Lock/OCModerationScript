...
(function ($, document, undefined) {
	...
	var posY = "175";
	var mpClick = false;
	var modFormat = "horizontal";
	var answers = false;
	var mp = false;
	var lastPage = false;
	var mpContent = false;
	
	promiseRequest("GET", answerFileLink)
		.then( response => {
			answers = JSON.parse(response.responseText);
		})
		.then( () => {
			// Mémorisation pages visitées
			GM.getValue("currentPage").then( response => {
				GM.setValue( "lastPage", response );
				lastPage = response;
				GM.setValue( "currentPage", window.location.pathname );
			});
			
			// Fermeture du sujet si demandée
			GM.getValue( "threadToLock" ).then( response => {
				if( response != '' && response !== undefined ) {
					promiseRequest("GET", response ).then( () => {
						GM.setValue( "threadToLock", '' );
					});
				}
			});
			
			// Suppression message si demandée
			GM.getValue( "postToDelete" ).then( response => {
				if( response != '' && response !== undefined ) {
					let deleteLink = baseUri + deleteUrl + response;
					let postData = '';
					promiseRequest("POST", deleteLink, postData ).then(() => {
						GM.setValue( "postToDelete", '' );
					});
				}
			});
			...
			// Traitement MP
			GM.getValue( "mpClick" ).then( response => {
				if( $("input#ThreadMessage_title").length && response ) {
					GM.setValue( "mpClick" , false );
					mp = answers.mp;
					GM.getValue( "mpContent" ).then( response => {
						mpContent = response;
						waitForTinymce( function(e) {
							let messageMp = mp.message.replace( '$$', lastPage ) + mpContent;
							$("input#ThreadMessage_title").val( mp.title );
							$("input#ThreadMessage_subtitle").val( lastPage.replace( messageUrl, "" ) );
			...
			if( $("input[name=submit_comment]").length ) {
				GM.getValue( "modFormat" ).then( response => {
					if( response !== undefined ) modFormat = response;
					else GM.setValue( "modFormat", modFormat );
				}).then( () => {
					GM.getValue('modPosX').then( response => {
						if( response !== undefined ) posX = response;
						else GM.setValue( "modPosX", posX );
					});
				}).then( () => {
					GM.getValue('modPosY').then( response => {
						if( response !== undefined ) posY = response;
						else GM.setValue( "modPosY", posY );
			...
			function initPost() {
				...
				if( nbMessages > 0 ) {
					$("#mainContentWithHeader").append(
						...
						'Messages possibles '+
						'<span class="oc-mod-version">'+answers.version+'</span>'+
						'</h3>'+
						'</div>'+
					...
					$("#oc-mod-panel").draggable({
						handle: "#oc-mod-drag",
						stop: function() {
							GM.setValue("modPosX", $(this).position().left );
							posX = $(this).position().left;
							GM.setValue("modPosY", $(this).position().top );
					...
					$("#oc-mod-formats").append( '<span class="oc-mod-tooltip" title="Permet de définir un affichage vertical de la boîte à outils"><input name="modFormat" type="radio" '+(modFormat == "vertical" ? 'checked="checked"' : "")+' value="vertical" /> Vertical</span>&nbsp;<span class="oc-mod-tooltip" title="Permet de définir un affichage horizontal de la boîte à outils"><input name="modFormat" type="radio" '+(modFormat == "horizontal" ? 'checked="checked"' : "")+' value="horizontal" /> Horizontal</span>' );
				...
				$(".oc-mod-mp").click( function(e) {
					GM.setValue( "mpContent", $(this).parent().parent().parent().find(".message.markdown-body").html() );
					GM.setValue( "mpClick" , true );
				});
				...
				$(".oc-mod-delete").click( function(e) {
					if( confirm( "Voulez-vous vraiment supprimer ce message ?" ) ) {
						GM.setValue( "postToDelete", $(this).parent().parent().parent().find(".span10.comment").attr("id").replace( 'message-', '' ) );
					} else {
						e.preventDefault();
					}
				});
				...
				$("input[name=modFormat]").click( () => {
					GM.setValue("modFormat", $("input[name=modFormat]:checked").val() );
					$("#oc-mod-panel").width(formats[modFormat]);
				});
				...
				$("#oc-mod-validation").click( () => {
					...
					if( moderationMessage.length ) {
						...
						if( $("input[name=shouldLock]").prop('checked') ) {
							GM.setValue( "threadToLock", baseUri + $(".closeAction").attr('href') );
